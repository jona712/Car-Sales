...
public class ParteBD {
    private final String RUTA_ARCHIVO = System.getProperty("user.dir")+ "\\src\\CapaArchivo\\Parte.txt";
    private FileOutputStream archivoSalida;
    private ObjectOutputStream oEscritor;
    private FileInputStream archivoEntrada;
    private ObjectInputStream oLector;
    private ArrayList<Parte> arrayParteTemp;
    //Instancia privada de la misma clase
    //implementa el patrón Singleton
    private static ParteBD instance = null;
    ...
    public static ParteBD getInstance() {
        if (instance == null) {
            instance = new ParteBD();
        }
        return instance;
    }
   ...
   //parámetro, en caso de que no lo encuentre retorna null
    public Parte consultarParte(String numParte)throws Exception {
        Parte parte,parteBuscado= null ;
        try {
            abrirArchivoInput();
            //Si no hay más datos que leer el método available retorna cero
             while(true){
                parte = (Parte)oLector.readObject();  //Lee un objeto a la vez             
                if(parte.getNumeroParte().equalsIgnoreCase(numParte)) {
                    parteBuscado = parte;
                }
             }            
        }catch(Exception e){
        ...
        try {
            ...
            if (oEscritor != null) {
              //Ejecutar la escritura del objeto departamento en el archivo
               oEscritor.writeObject(parte); //Serializa el objeto y lo graba en el archivo
     ...
     */
      public void modificarParte(Parte parte) throws Exception{
        arrayParteTemp = new ArrayList<Parte>();
        try {            
            abrirArchivoInput();            
            ...
            //Si no hay más datos que leer el método available retorna cero
             while(true){//Si va a leer y no hay objeto Departamento se va por el catch
                 parte1 = (Parte)oLector.readObject(); //Si lee y no hay deparyamento que leer se cae
                 if(parte1.getNumeroParte().equalsIgnoreCase(parte.getNumeroParte())) {
                   parte1=parte;
                 }
                 arrayParteTemp.add(parte1);
             }  
        }
        catch(Exception ex){
    ...
    public void eliminarParte(String numParte) throws Exception {
        arrayParteTemp = new ArrayList<Parte>();
        try {            
            abrirArchivoInput();
            Parte parte = null;
